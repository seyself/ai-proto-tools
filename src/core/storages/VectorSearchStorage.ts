...
// 定数定義
const EMBEDDING_MODEL_SMALL: EmbeddingModel = { name: 'text-embedding-3-small', size: 1536 };
const EMBEDDING_MODEL_LARGE: EmbeddingModel = { name: 'text-embedding-3-large', size: 3072 };
const EMBEDDING_MODEL_OLD: EmbeddingModel = { name: 'text-embedding-ada-002', size: 1536 };
...
export default class VectorSearchStorage
{
  ...
  private static _db: Record<string, VectorSearchStorage> = {};
   ...
   */
  public getIdByKey = (key: string): number | null => {
    try {
      const query = this.db?.prepare(`
        ...
        LIMIT 1
      `);
      const result = query?.get(key) as { id: number } | undefined;
   ...
   */
  public save = async (content: string, metaData: Record<string, any> | null = null): Promise<void> =>
  {
    if (!this.db) throw new Error('Database not initialized');
    try
    {
      ...
      if (!last_id) throw new Error('Failed to get ID for key: ' + key);
   ...
   */
  public getItemByKey = (key: string): StorageItem | null => {
    if (!this.db) throw new Error('Database not initialized');
    try {
      const query = this.db.prepare(`
        SELECT content, meta
        ...
        LIMIT 1
      `);
      const result = query.get(key) as { content: string; meta: string } | undefined;
      if (result) {
        return {
   ...
   */
  public search = async (content: string, len=10): Promise<SearchResult[]> =>
  {
    if (!this.db) throw new Error('Database not initialized');
    try
    {
 ...
 */
export async function vectorizeText(text: string, model=''): Promise<number[]> {
  try 
  {
